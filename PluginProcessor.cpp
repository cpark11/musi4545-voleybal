...
Mu45effectAudioProcessor::Mu45effectAudioProcessor()
{
	...
	//balance param
	addParameter(balanceUParam = new PluginParameter("balance",     // parameterId
		0.0,         // actualDefaultValue
		-50.0,         // actualMinimum
		50.0,        // actualMaximum
...
{
	//change volume so that perceived sound is louder 
    gainAParam = powf(10.0, volumeUParam->getActualValue()/20.0);

	//calculate balance panner where Rgain + Lgain = 1 linear option
	//Rgain = Pn
	//Lgain = 1 - Rgain;
	//Pn = 0.5 + Pan*1/100
	...
	//balance param
	float pn = M_PI / 4 + balanceUParam->getActualValue()*(M_PI / 200);
	rGain = sin(pn);
	lGain = cos(pn);
}
...
void Mu45effectAudioProcessor::processBlock (AudioSampleBuffer& buffer, MidiBuffer& midiMessages)
{
    ...
    {
        left[samp] = gainAParam * left[samp] * lGain; //added balance panner for left
        right[samp] = gainAParam * right[samp] * rGain; //added balance panner for right
    }
}
